...
cd src && \
  docker run --rm \
    ...
    'apt-get update && \
      ...
      make && \
      mkdir -p out && \
      /emsdk_portable/emscripten/tag-1.38.43/emcc -Oz --bind pwgen.o pw_phonemes.o pw_rand.o randnum.o sha1.o sha1num.o -s STRICT=1 -s ALLOW_MEMORY_GROWTH=1 -s MALLOC=emmalloc -s ENVIRONMENT=web -s MODULARIZE=1 -s EXPORT_ES6=1 -s EXIT_RUNTIME=1 -s EXPORT_NAME="pwgen" -o out/pwgen.html'
